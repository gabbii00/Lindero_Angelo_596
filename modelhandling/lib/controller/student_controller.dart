...
class StudentController {
  ...
  List<Student> searchStudents(List<Student> student, String query) {
    // YOUR CODE HERE
    if (query.isEmpty) return student;
    return student.where((student) => student.name.toLowerCase().contains(query.toLowerCase())).toList();
  }

  // TO DO: Calculate the class average
  // Sum all student averages and divide by total students
  // Return 0 if list is empty
  double getClassAverage(List<Student> student) {
    // YOUR CODE HERE
    if (student.isEmpty) return 0.0;
    double total = student.fold(0.0, (sum, student) => sum + student.average);
